...
public class UpdateRfi extends CustomTitle{
	...
	/*private String rf_struct_id;
	private String rf_stage_id;
	private String rf_unit_id;
	private String rf_subunit_id;
	private String rf_element_id;
	private String rf_subelmt_id;
	private String rf_checklist_id;
	private String rf_grp_id;
	private String rf_node_id;
	private String rf_project_id;*/
...
//			logout();
	...
	/*protected boolean validateScreen() {
		...
		if (schemeSpin.getSelectedItemPosition() == 0) {
		...
		} else if (checkLististSpin.getSelectedItemPosition() == 0) {
			errorMessage = "Please Select Check List";
		} else if (subgrpSpin.getSelectedItemPosition() == 0) {
			errorMessage = "Please Select Sub Group";
		} else if (db.contr && contractorspin.getSelectedItemPosition() == 0) {
			errorMessage = "Please Select Contractor";
		}else if (db.contractor_flag && contractorspin.getSelectedItemPosition() == 0) {
			errorMessage = "Please Allocate Contractor";
		} else if (supervisorspin.getSelectedItemPosition() == 0
				&& foremanspin.getSelectedItemPosition() == 0) {
			errorMessage = "Please Select Atleast Supervisor or Foreman";
		}else if ((db.supervisor_flag) 
				&& foremanspin.getSelectedItemPosition() == 0) {
			errorMessage = "Please Select Foreman";
		}else if (supervisorspin.getSelectedItemPosition() == 0 
				&& db.foreman_flag foremanspin.getSelectedItemPosition() == 0) {
			...
			errorMessage = "Please Select client";
				...
				&& db.foreman_flag) {
			errorMessage = "Please Select Supervisor or Allocate Foreman";
		}else
			validate = false;
		db.contr= false;
		db.supervisor_flag= false;
		db.foreman_flag= false;
		...
		rfiSpin.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> aview, View view,
				...
				if (position >0) {
					...
					System.out.println("iddddddddd====="+r_clint_id[position-1]+"=="+r_prj_id[position-1]);
...
//----------checking worktype id Subunit--------------
					...
					if(CheckWorkType(table44, where44) && Integer.parseInt(db.selectedlevelId)>=5)
					{
						//setFloorSpinnerData(r_struct_id[position-1], r_prj_id[position-1],r_stage_id[position-1],r_worktype_id[position-1]);
						//setUnitSpinnerData(r_stage_id[position-1],r_prj_id[position-1],r_unit_id[position-1],r_worktype_id[position-1]);
						setElementSpinnerData(r_subunit_id[position-1], r_prj_id[position-1],r_elemnt_id[position-1],r_worktype_id[position-1]);
					}
					//---------end Element----------
...
//----------checking worktype id Subelement--------------
					...
					if(CheckWorkType(table55, where55) && Integer.parseInt(db.selectedlevelId)>=6)
					{
						//setFloorSpinnerData(r_struct_id[position-1], r_prj_id[position-1],r_stage_id[position-1],r_worktype_id[position-1]);
						//setUnitSpinnerData(r_stage_id[position-1],r_prj_id[position-1],r_unit_id[position-1],r_worktype_id[position-1]);
						setSubElementSpinnerData(r_elemnt_id[position-1], r_prj_id[position-1],r_subelmt_id[position-1],r_worktype_id[position-1]);
					}
					//---------end SubElement----------
					...
					/*setFloorSpinnerData(r_struct_id[position-1], r_prj_id[position-1],r_stage_id[position-1]);
					setUnitSpinnerData(r_stage_id[position-1],r_prj_id[position-1],r_unit_id[position-1]);
					setSubUnitSpinnerData(r_unit_id[position-1],r_prj_id[position-1],r_subunit_id[position-1]);
					
					setElementSpinnerData(r_subunit_id[position-1], r_prj_id[position-1],r_elemnt_id[position-1]);
					
					setSubElementSpinnerData(r_elemnt_id[position-1], r_prj_id[position-1],r_subelmt_id[position-1]);
					setCheckListSpinnerData(r_prj_id[position-1],r_checklist_id[position-1]);
					setGoupSpinnerData(r_checklist_id[position-1],r_grp_id[position-1],r_node_id[position-1]);*/
					...
					System.out.println("selected all id project="+r_prj_id[position-1]+" struct="+r_struct_id[position-1]+
							...
							" el="+r_elemnt_id[position-1]+"sub ele="+r_subunit_id[position-1]+" check ="+r_checklist_id[position-1] +" grop="+r_grp_id[position-1]
					...
					setSpiner(); 
					//coverageedit.setHint(r_coverage_id[position-1]); 
	...
	private void setSchemeSpinnerData(String Cl_ID,String rf_project_id){
		...
		/*//PK_Scheme_ID TEXT,Scheme_Name TEXT,Scheme_Cl_Id TEXT,Scheme_Diplay_Name TEXT,Scheme_Adrs TEXT," +
		"Scheme_Region TEXT, user_id TEXT
*/		
		...
		projSpin.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> aview, View view,
				...
				if (position > 0) {
					...
					System.out.println("heloooooooo===="+db.selectedSchemeName);
	...
	private void setBulidngSpinnerData(final String schemeid,String rf_struct_id,String wrktype_id) {
		...
		// Building(Bldg_ID TEXT,Bldg_Name TEXT,Build_scheme_id TEXT, user_id TEXT)");
...
private void setSubUnitSpinnerData(String unit_id,final String schemid,String rf_subunit_id,String wrkType_id) {
		...
		// TABLE Unit(Unit_ID TEXT,Unit_Des TEXT,Unit_Scheme_id TEXT,Fk_Floor_ID TEXT, user_id TEXT)");
		//SubUnit(Sub_Unit_ID TEXT,Sub_Unit_Des TEXT,Sub_Unit_Scheme_id TEXT,FK_Unit_ID TEXT, user_id TEXT)");
	String subunt_id="";
...
private void setElementSpinnerData(String subunit_id,final String schemid,String rf_element_id,String wrkType_id) {
	
	
	//  Element(Elmt_ID TEXT,Elmt_Des TEXT,Elmt_Scheme_id TEXT,FK_Sub_Unit_ID TEXT, user_id TEXT)");
	
String elemnt="";

	String where = "e.Elmt_Scheme_id=s.PK_Scheme_ID"
			+"  AND s.user_id='" + db.userId 
			+"'  AND e.FK_Sub_Unit_ID='" + subunit_id
	...
	Cursor cursor = db.select("Element as e,Scheme as s",
			"distinct(e.Elmt_ID),e.Elmt_Des", where, null, null,
			null, "e.Elmt_Des");
	...
	elementspin.setOnItemSelectedListener(new OnItemSelectedListener() {
		...
		public void onItemSelected(AdapterView<?> aview, View view,
			...
			//	setSubElementSpinnerData(elementId[position - 1],schemid);
				/*subelementspin.setClickable(true);
				subelementspin.setSelection(0);*/
				System.out.println("selected el sub id"+ elementId[position - 1]);
...
private void setSubElementSpinnerData(String element_id,final String schemid,String rf_subelmt_id,String wrkType_id) {
	
	String subelmnt="";
	//  Element(Elmt_ID TEXT,Elmt_Des TEXT,Elmt_Scheme_id TEXT,FK_Sub_Unit_ID TEXT, user_id TEXT)");
	
//SubElement(Sub_Elmt_ID TEXT,Sub_Elmt_Des TEXT,Sub_Elmt_Scheme_id TEXT,FK_Elmt_ID TEXT, user_id TEXT)");

	String where = "e.Sub_Elmt_Scheme_id=s.PK_Scheme_ID"
			+" AND s.user_id='" + db.userId 
			+"' AND s.PK_Scheme_ID='" + schemid
			+"' AND e.FK_Elmt_ID='" + element_id
			+"' AND e.FK_WorkTyp_ID='" + wrkType_id
	...
	subelementspin.setOnItemSelectedListener(new OnItemSelectedListener() {
		...
		public void onItemSelected(AdapterView<?> aview, View view,
			...
			if (position > 0) {
				...
				System.out.println("selected subel sub id"+ subelementId[position - 1]);
	...
	// CheckList(Checklist_ID TEXT,Checklist_Name TEXT,Node_Id TEXT, user_id TEXT)");
...
// Group1(Grp_ID TEXT,Grp_Name TEXT,Node_id TEXT,FK_Checklist_ID TEXT,user_id TEXT)");

	String grp_id="";		
	
	/*String where = "c.Checklist_ID='"
			+ checklistId
			+ "'  AND  g.FK_Checklist_ID=c.Checklist_ID"+" AND c.user_id='" + db.userId +"' AND g.Grp_ID='" + rf_grp_id+"' AND g.Node_id='" + node_id
...
if(setspinflag){
	...
	public void saveData(String data) { 
		if(data.equalsIgnoreCase("No Record Found")){
			...
			/*	column = "Bldg_ID,Bldg_Name,Build_scheme_id,user_id";//buildng 
				saveToDatabase("Building", column, tabledata[2],true,3);
 
				
				column = "floor_Id,floor_Name,Floor_Scheme_ID,FK_Bldg_ID, user_id";//buildng
				saveToDatabase("floor", column, tabledata[3],true,4);
				...
				column = "Unit_ID,Unit_Des,Unit_Scheme_id,Fk_Floor_ID,user_id";//buildng
				saveToDatabase("Unit", column, tabledata[4],true,4);
				
				
				
				column = "Sub_Unit_ID,Sub_Unit_Des,Sub_Unit_Scheme_id,FK_Unit_ID,user_id";//buildng
				saveToDatabase("SubUnit", column, tabledata[5],true,4);

				
				column = "Elmt_ID,Elmt_Des,Elmt_Scheme_id,FK_Sub_Unit_ID, user_id";//buildng
				saveToDatabase("Element", column, tabledata[6],true,4);
				
				 
				column = "Sub_Elmt_ID,Sub_Elmt_Des,Sub_Elmt_Scheme_id,FK_Elmt_ID,user_id";//buildng
				saveToDatabase("SubElement", column, tabledata[7],true,4);
				
				
				
				
							
				column = "Checklist_ID,Checklist_Name,Node_Id,user_id";//buildng
				...
				saveToDatabase("question", column, tabledata[10],true,7);
				*/
	...
	public void saveToDatabase1(String table, String columns, String respose, boolean adduserId, int colCnt) {
		...
		for (int len = 0; len < rowdata.length; len++) {
			...
			for (int i = 0; i < ( singlerow.length - 1); i++) {
				values.append( singlerow[i] + "','");
				/*System.out.println("before---"+ singlerow[i]);
				String temp=singlerow[i].replace(",","/");
				values.append( temp + "','");
			...
			values.append( singlerow[ singlerow.length - 1 ] + "'");
			 ...
			 if(adduserId)
				...
				// Rfi_name TEXT,Image3 TEXT,image4 TEXT,checker_remark TEXT,drawing_no TEXT,status_device TEXT,user_id1 TEXT/
