...
public class CheckQuestionire extends CustomTitleDuplicate {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.check_questionire);
		...
		Cursor cursor = db.select("question as q,Check_update_details r", "q.PK_question_id,q.QUE_Des,q.QUE_SequenceNo,q.QUE_Type",whereClause, null, null, null, null);
	...
	*/
			...
			public void onClick(View v) {
				...
				if(imageCount < 2){
					...
					try {
						...
						String imageName ="checker"+"_"+ g+"_"+db.selectedNodeId.trim()+"_"+db.selectedChecklistId.trim()+"_"+
								db.selectedGroupId.trim()+"_"+db.userId.trim()+"_"+current_ques_id
								+"_"+question_seq_id+"_"+question_type+ "_" + d.getTime()+"_"+imageCount+".jpg";
						
						
						
					/*	if (!snaps[2].equalsIgnoreCase("") && !snaps[3].equalsIgnoreCase("")){
							snaps[2] = imageName;
							System.out.println("second.. cinduition.........");	
						} else if(!snaps[2].equalsIgnoreCase("") && imageCount==1){
							snaps[3] = imageName;
					...
					*/		System.out.println("third cinduition.........");
						...
						/*if(imageCount==0){
							...
							System.out.println(" image  name 1"+snaps[2]);
						}else{
							snaps[3] = imageName;
							System.out.println(" image  name 2"+snaps[3]);
						}*/
						...
						System.out.println("snaps====="+snaps[2]);
	...
	public void nevigateHomeScreen(){
		...
		alertDialog.setButton("Ok", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				deleteAnswer();
				CheckFRI.destructcheckRfi.finish();
				finish();
				Intent int1=new Intent(CheckQuestionire.this,HomeScreen.class);
	...
	private void getQuestionData(int sr,String Q_id) {
		...
		questionAnswerCursor = db.select("CheckAnswer", "ans_text,remark,snap3,snap4",whereClause121, null, null, null, null);
		int cnt=0;
		if(questionAnswerCursor.moveToFirst()){
			do{
				isanwered=true;
				System.out.println("question data Check questionire....");
				...
				System.out.println("snap1...."+questionAnswerCursor.getString(questionAnswerCursor.getColumnIndex("snap3")));
				System.out.println("snap2...."+questionAnswerCursor.getString(questionAnswerCursor.getColumnIndex("snap4")));
				cnt++;
				...
				TempsnapLoacl1=questionAnswerCursor.getString(questionAnswerCursor.getColumnIndex("snap3"));
				TempsnapLoacl2=questionAnswerCursor.getString(questionAnswerCursor.getColumnIndex("snap4"));
	...
	private void SingleChoiceLayout() {
		...
		for(int i=0; i<options.length; i++){
			...
			try{
				...
				p_checker_rem = questionupdateCursor.getString(questionupdateCursor.getColumnIndex("checker_remark"));
			}
			catch(Exception e)
			{
		...
		InputFilter filter = new InputFilter() {
			...
			public CharSequence filter(CharSequence source, int start, int end,
				...
				if(dstart > 999){ // limit 1000 characters for edit text
					return "";
				}
				return null;
			}
...
*/
	...
	protected void saveAnswer() {
		...
		System.out.println("images are :::"+snaps[2]+snaps[3]);
		//questionData[Qdata]="";
	...
	protected void getScreenToBeDisplayed() {
		...
		cleanData();
		db.closeDb();
		CheckFRI.Checkclass.finish();
		finish();

		//startActivity(new Intent(Update_Questionire.this,HomeScreen.class));
		Intent inte1=new Intent(CheckQuestionire.this,HomeScreen.class);
	...
	private void showImage() {
		...
		if (snaps[2].equalsIgnoreCase("")) {
			...
			AlertDialog.Builder builder = new AlertDialog.Builder(
					CheckQuestionire.this, R.style.MyAlertDialogStyle);
			...
			Intent i = new Intent(CheckQuestionire.this, ViewImagesActivity.class);
	...
	private void logout() {
		...
		finish();
		Toast.makeText(CheckQuestionire.this, "Session expired... Please login.", Toast.LENGTH_SHORT).show();
		Intent logout = new Intent(CheckQuestionire.this, LoginScreen.class);
