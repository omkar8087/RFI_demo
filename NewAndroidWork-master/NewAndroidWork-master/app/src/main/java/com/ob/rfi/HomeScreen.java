...
public class HomeScreen extends CustomTitle {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Button allocateTask = (Button) findViewById(R.id.allocate_task);
        Button startInspectButton = (Button) findViewById(R.id.start_ins);
        Button updaterfi = (Button) findViewById(R.id.updaterfi);
        Button allocateUnitButton = (Button) findViewById(R.id.check_rfi);
        Button testRfi = (Button) findViewById(R.id.test_rfi);
        ...
        System.out.println("checking value=" + rollval);
        try {
            if (rollval.equalsIgnoreCase("maker")) {
                System.out.println("checking value1=" + rollval);
                allocateUnitButton.setEnabled(false);
                System.out.println("Selected UserID = " + db.userId);
...
//            displayDialog("Data Available", "Do you want to clear Data");
    ...
    private void builduploadData() {
        ...
        if ((isRFIDetailsAvilable()) || (ischeckRFIDetailsAvilable() && RfiDatabase.isToBeUploadedCheck) || iscancelRFIDataAvailable()) {
            ...
            if (ischeckRFIDetailsAvilable()) {
				...
				/*RFI_Id~CL_Id~PRJ_Id~NODE_Id~Level_int~CHKL_Id~GRP_Id~USER_Id~Structure_Id~Stage_Id~Unit_id~Sub-Unit_Id~Element_Id
				~SubElement_Id~CreatedOn_date~Coverage_str~QUE_Id~Answer_string~Remark_string~ImageFile1~ ImageFile2$
				|
				RFI_Id~USER_Id~CheckedOn_date~QUE_Id~Que_status(OK/Send Back)$
				*/
            ...
            uploadData(uploadanswerRFIData + "|" + uploadanswercheckRFIData + "|" + uploadCancelRFIData);
